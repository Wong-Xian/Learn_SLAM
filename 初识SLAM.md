...
2. 理解一个空节点是如何投影到相机成像平面的
3. 掌握 OpenCV 的图像存储与表达方式
4. 学会基本的摄像头标定方法

- 5.1 相机模型
  - 相机把三维世界的坐标点（单位米）映射到二维平面（单位像素）的过程可用几何模型描述。
  - 最经典：针孔模型：一束光经过针孔，在后面投影成像
  - 注意：经过透镜，图像会产生畸变。
  - 5.1.1 针孔相机模型
    - 小孔成像
    - 相似三角形
    - 定义现实坐标系（O为相机光心，z轴是相机指向的方向，x向右，y向下）和像素坐标系（o'位于图像左上角，u轴向右与x轴平行，v轴向下与y轴平行），现实坐标系与像素坐标系之间存在转换关系。进而可写成矩阵表示。进而引出相机的内参数矩阵K。
    - 相机的标定：确定内参数矩阵K的过程。
  - 5.1.2 畸变
    - 畸变来源：
      - 透镜对光的折射（径向畸变）通过k1，k2，k3参数矫正
        - 桶形畸变：离光轴越远，图像放大率越小。
        - 枕形畸变：离光轴越远，图像放大率越大。
      - 组装透镜时不可能与成像平面完全平行（切向畸变）通过p1，p2参数矫正
  - 5.1.3 双目相机模型
    - 通过视差，推算每一个像素的深度。
    - 缺点：对计算机来说计算量很大，计算复杂。
  - 5.1.4 RGB-D相机模型
...
本章目标：
1. 理解最小二乘法的含义和处理方式
2. 理解 高斯牛顿法  列文伯格马夸尔特法 等下降策略
3. 学习 Ceres 库和 g2o 库的基本使用方法

- 6.1 状态估计问题
  - 6.1.1 最大后验与最大似然
    - 最大后验：利用经验数据，对未观测点的状态进行估计
    - 最大似然：在什么样的状态下，最有可能产生现在观测到的数据
    - 增量/渐进方法（滤波器）：持有一个当前时刻的估计状态，并用新的数据来更新它
    - 批量方法：把数据“攒”起来一起处理
